...
class MysqlTableTest extends TestCase
{
    ...
    public function testXorConditions(): void
    {
        $table = $this->db->table('test');
    ...
    public function testSumWithReservedWordColumn(): void
    {
        $this->assertNotFalse($this->db->execute('CREATE TABLE foobar (`order` INTEGER)'));
        $this->assertTrue($this->db->table('foobar')->insert(['order' => 3]));
        $this->assertTrue($this->db->table('foobar')->insert(['order' => 4]));
